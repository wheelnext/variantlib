...
def test_namespace_missing_module():
    with pytest.raises(
        PluginError,
        match=r"Loading the plugin from plugin api 'tests.no_such_module:foo' failed: "
...
def test_namespace_incorrect_name():
    with pytest.raises(
        PluginError,
        match=r"Loading the plugin from plugin api 'tests.test_plugins:no_such_name' "
...
def test_namespace_incorrect_type():
    with pytest.raises(
        PluginError,
        match=r"Plugin api 'tests.test_plugins:RANDOM_STUFF' points at a value that "
...
def test_namespace_instantiation_raises():
    with pytest.raises(
        PluginError,
        match=r"Instantiating the plugin from plugin api "
...
def test_namespace_instantiation_returns_incorrect_type(cls: type, mocker):
    with pytest.raises(
        PluginError,
        match=rf"Instantiating the plugin from plugin api 'tests.test_plugins:{cls}' "
