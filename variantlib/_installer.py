...
def run_subprocess(cmd: list[str], env: Mapping[str, str] | None = None) -> bool:
    try:
...
class IsolatedEnv(typing.Protocol):
    """Isolated variant plugin environment ABC."""
...
def _has_dependency(
    ...
    sufficient for `variantlib`, False if it is not, None if the package is missing.
    """
...
class DefaultIsolatedEnv(IsolatedEnv):
    ...
    def __enter__(self) -> DefaultIsolatedEnv:  # noqa: PYI034
        try:
            path = tempfile.mkdtemp(prefix="variant-env-")
    ...
    @property
    def path(self) -> str:
        """The location of the isolated variant plugin environment."""
        return self._path

    @property
    def python_executable(self) -> str:
        """The python executable of the isolated variant plugin environment."""
...
class CurrentPythonEnv(IsolatedEnv):
    """
    ...
    def __enter__(self) -> CurrentPythonEnv:  # noqa: PYI034
        try:
            self._env_backend: _EnvBackend
    ...
    @property
    def python_executable(self) -> str:
        """The python executable of the current environment."""
...
class _PipBackend(_EnvBackend):
    ...
    def _has_virtualenv(self) -> bool:
        ...
        from `variantlib`. This verifies that virtualenv and all of its
        dependencies are installed as required by `variantlib`.
        """
        ...
        return importlib.util.find_spec(name) is not None and not any(
            ...
            for d in check_dependency(f"variantlib[{name}]")
            if len(d) > 1
        )
    ...
    def create(self, path: str) -> None:
        if self._create_with_virtualenv:
            ...
            if with_pip:
                ...
                # Uninstall setuptools from the variant plugin env to prevent depending
                # on it implicitly. Pythons 3.12 and up do not install setuptools,
    ...
    def install_requirements(self, requirements: Collection[str]) -> None:
        ...
        with tempfile.NamedTemporaryFile(
            "w", prefix="variants-reqs-", suffix=".txt", delete=False, encoding="utf-8"
        ) as req_file:
